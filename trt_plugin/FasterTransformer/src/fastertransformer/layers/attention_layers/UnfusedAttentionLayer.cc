                                       ...
                                       const AttentionWeight<T>* attention_weights)
{
    ...
    if (sparse_ && cublas_wrapper_->isUseSparse(1, n, m, k)) {
        ...
        // std::printf("is_batched_QKV_ = %d \n", (int)is_batched_QKV_);
        // std::printf("cublas_wrapper_->getStream() = %p \n", cublas_wrapper_->getStream());
        if (is_batched_QKV_) {
            ...
            cublas_wrapper_->Gemm(CUBLAS_OP_N,
                                  ...
                                  q_buf_,
                                  n);
            // 这里等于 q_buf_ = from_tensor @ attention_weights->query_weight.kernel
            // 验证 from_tensor 与 layernorm 输出是否相同
            ...
            cublas_wrapper_->Gemm(CUBLAS_OP_N,
                                  ...
                                  n);
            // if(0 == count_for_test) {
            //     std::printf("test for cublas_wrapper_->Gemm\n");
            //     Tensor(MEMORY_GPU, TYPE_FP32, {k, n}, attention_weights->query_weight.kernel).save<float>("/target/mobilebit_trt/FasterTransformer/build/weights/q_weight.npy");
            //     Tensor(MEMORY_GPU, TYPE_FP32, {k, n}, attention_weights->key_weight.kernel).save<float>("/target/mobilebit_trt/FasterTransformer/build/weights/k_weight.npy");
            //     Tensor(MEMORY_GPU, TYPE_FP32, {k, n}, attention_weights->value_weight.kernel).save<float>("/target/mobilebit_trt/FasterTransformer/build/weights/v_weight.npy");
            //     Tensor(MEMORY_GPU, TYPE_FP32, {m, k}, from_tensor).save<float>("/target/mobilebit_trt/FasterTransformer/build/weights/input.npy");
            //     Tensor(MEMORY_GPU, TYPE_FP32, {m, n}, q_buf_).save<float>("/target/mobilebit_trt/FasterTransformer/build/weights/q.npy");
            //     Tensor(MEMORY_GPU, TYPE_FP32, {m, n}, k_buf_).save<float>("/target/mobilebit_trt/FasterTransformer/build/weights/k.npy");
            //     Tensor(MEMORY_GPU, TYPE_FP32, {m, n}, v_buf_).save<float>("/target/mobilebit_trt/FasterTransformer/build/weights/v.npy");
            //     ++count_for_test;
            // }
        }
#ifdef SPARSITY_ENABLED
