                          ...
                          const FfnWeight<T>* ffn_weights)
{
    ...
    // if(0 == check_cnt) {
    //     std::printf("m = %d, inter_size_ = %d, hidden_units_ = %d \n", m, inter_size_, hidden_units_);

    //     // hidden_units_, inter_size_
    //     Tensor t1{MEMORY_GPU, TYPE_FP32, std::vector<size_t>{hidden_units_, inter_size_}, ffn_weights->intermediate_weight.kernel};
    //     t1.save<T>("/target/mobilebit_trt/FasterTransformer/build/weights/ffn_inter_weight.npy");
        
    //     // inter_size_
    //     Tensor t2{MEMORY_GPU, TYPE_FP32, std::vector<size_t>{inter_size_}, ffn_weights->intermediate_weight.bias};
    //     t2.save<T>("/target/mobilebit_trt/FasterTransformer/build/weights/ffn_inter_bias.npy");

    //     Tensor t3{MEMORY_GPU, TYPE_FP32, std::vector<size_t>{m, inter_size_}, inter_buf_};
    //     t3.save<T>("/target/mobilebit_trt/FasterTransformer/build/weights/ffn_inter_out.npy");
...
/// 2022/06/01 add start (feed forward network)

template<typename T>
SwishFfnLayer<T>::SwishFfnLayer(size_t max_batch_size,
                              size_t max_seq_len,
                              size_t head_num,
                              size_t size_per_head,
                              size_t inter_size,
...
template<typename T>
SwishFfnLayer<T>::SwishFfnLayer(SwishFfnLayer<T> const& swish_ffn_layer): FfnLayer<T>(swish_ffn_layer)
{
}

template<typename T>
void SwishFfnLayer<T>::invokeAddBiasActivation(const int m, const T* bias)
{
    invokeAddBiasSwish<T>(inter_buf_, bias, m, inter_size_, stream_);
}

template class SwishFfnLayer<float>;
template class SwishFfnLayer<half>;
#ifdef ENABLE_BF16
template class SwishFfnLayer<__nv_bfloat16>;
#endif

/// 2022/06/01 add end (feed forward network)
